...
class ArrayExtensionViewController: TestBaseViewController {
...
extension ArrayExtensionViewController {
    ...
    @objc func test51() {
        ...
        print("数组转字符转（数组的元素是 字符串）", "数组：\(testArray) 转为字符串为：\(testString)")
    }
}
...
extension ArrayExtensionViewController {
    ...
    @objc func test41() {
        var testArray = ["1".jx.toNSString, "2".jx.toNSString, "3".jx.toNSString, "2".jx.toNSString]
        let oldArray = testArray
        let element = "2".jx.toNSString
        let newArray = testArray.remove(object: element, isRepeat: false)
        print("删除数组中遵守NSObjectProtocol协议的元素，是否删除重复的元素)", "原数组为：\(oldArray) 删除其中的值：\(element) 后数组为：\(newArray)")
    }
    
    // MARK: 4.2、删除一个遵守NSObjectProtocol的数组中的元素，支持重复删除
    @objc func test42() {
        var testArray = ["1".jx.toNSString, "2".jx.toNSString, "3".jx.toNSString, "2".jx.toNSString]
        let removeArray = ["2".jx.toNSString, "3".jx.toNSString]
        let oldArray = testArray
        let newArray = testArray.removeArray(objects: removeArray, isRepeat: true)
        print("删除一个遵守NSObjectProtocol的数组中的元素，支持重复删除", "原数组为：\(oldArray) 删除的数组是：\(removeArray) 后数组为：\(newArray)")
    }
}
...
extension ArrayExtensionViewController {
    ...
    @objc func test31() {
        ...
        print("删除数组的中的元素(可删除第一个出现的或者删除全部出现的)", "原数组为：\(oldArray) 删除其中的值：\(element) 后数组为：\(newArray)")
    }
    ...
    @objc func test32() {
        ...
        print("从删除数组中删除一个数组中出现的元素，支持是否重复删除, 否则只删除第一次出现的元素", "原数组为：\(oldArray) 删除的数组是：\(removeArray) 后数组为：\(newArray)")
    }
}
...
extension ArrayExtensionViewController {
    ...
    @objc func test21() {
        ...
        let element = "2"
        print("获取数组中的指定元素的索引值", "查找 \(testArray) 中的 \(element) 的索引为：\(testArray.indexes(element))")
    }
    ...
    @objc func test22() {
        let testArray = ["1", "2", "3", "2"]
        let element = "2"
        print("获取元素首次出现的位置", "\(element) 在数组：\(testArray) 首次出现的索引是：\(testArray.firstIndex(element) ?? 0)")
    ...
    @objc func test23() {
       let testArray = ["1", "2", "3", "2"]
       let element = "2"
       print("获取元素最后出现的位置", "\(element) 在数组：\(testArray) 最后出现的索引是：\(testArray.lastIndex(element) ?? 0)")
...
extension ArrayExtensionViewController {
    ...
    @objc func test14() {
        ...
        print("分隔数组", "原数组：\(array) 分隔后为：\(parts)")
    }
    
    // MARK: 1.3、数组 -> JSON字符串
    @objc func test13() {
        let array = [["a": "1"], "2"] as [Any]
        print("数组 -> JSON字符串", "数组：\(array) 转为JSON字符串为：\(array.toJSON() ?? "数组 ❌ JSON字符串")")
    }
    ...
    @objc func test12() {
        ...
        print("数组新增元素(可转入一个数组)", "原数组是： \(oldArray) 添加 \(addArray) 后为 \(testArray)")
    }
    ...
    @objc func test11() {
        ...
        guard let value = testArray.indexValue(safe: index) else {
            print("取数组 \(testArray) 索引为 \(index) 是没有值的")
            return
        }
        print("安全的取某个索引的值", "在数组 \(testArray) 中获取索引 \(index) 的值是 \(value)")
    }
}
