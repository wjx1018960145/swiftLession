...
class UIButtonExtensionViewController: TestBaseViewController {
...
extension UIButtonExtensionViewController {
    ...
    @objc func test61() {
        ...
        let button1 = UIButton(frame: CGRect(x: 25, y: 115, width: 50, height: 50)).image(.brown).title("哈哈").jx.setImageTitleLayout(.imgLeft, spacing: 6)
        button1.backgroundColor = .randomColor
        button1.tag = 100
        button1.jx.setHandleClick { (btn) in
            guard let weakBtn = btn else { return }
            print("button的事件", "tag：\(weakBtn.tag)")
        }
        button1.jx.expandSize(size: 15)
        ...
        let button2 = UIButton(frame: CGRect(x: 145, y: 115, width: 50, height: 50)).image(.brown).title("哈哈").jx.setImageTitleLayout(.imgLeft, spacing: 6)
        button2.backgroundColor = .randomColor
        button2.tag = 150
        button2.jx.expandSize(size: 7.5)
        button2.jx.setHandleClick { (btn) in
            guard let weakBtn = btn else { return }
            print("button的事件", "tag：\(weakBtn.tag)")
        }
...
extension UIButtonExtensionViewController {
    ...
    // MARK: 5.1、button的事件
    @objc func test51() {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 100, height: 100)).image(.brown).title("哈哈").jx.setImageTitleLayout(.imgLeft, spacing: 6)
        button.backgroundColor = .randomColor
        button.center = self.view.center
        button.tag = 100
        button.jx.setHandleClick { (btn) in
            guard let weakBtn = btn else { return }
            print("button的事件", "tag：\(weakBtn.tag)")
        }
...
extension UIButtonExtensionViewController {
    ...
    // MARK: 3.2、图片在右
    @objc func test32() {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 100, height: 100)).image(.brown).title("哈哈").jx.setImageTitleLayout(.imgRight, spacing: 6)
        button.backgroundColor = .randomColor
        button.center = self.view.center
        self.view.addSubview(button)
        JXAsyncs.asyncDelay(3, {
    ...
    // MARK: 3.3、图片在上
    @objc func test33() {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 100, height: 100)).image(.brown).title("哈哈").jx.setImageTitleLayout(.imgTop, spacing: 6)
        button.backgroundColor = .randomColor
        button.center = self.view.center
        self.view.addSubview(button)
        JXAsyncs.asyncDelay(3, {
    ...
    // MARK: 3.4、图片在下
    @objc func test34() {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 100, height: 100)).font(6).image(.brown, CGSize(width: 20, height: 20), .normal).title("哈哈").jx.setImageTitleLayout(.imgBottom, spacing: 6)
        button.backgroundColor = .randomColor
        button.center = self.view.center
        self.view.addSubview(button)
        JXAsyncs.asyncDelay(3, {
...
extension UIButtonExtensionViewController {
    ...
    @objc func test27() {
        ...
        guard let path = Bundle.init(for: Self.self).path(forResource: "jxBaseKit", ofType: "bundle") else {
            return
        }
...
extension UIButtonExtensionViewController {
    ...
    @objc func test11() {
        let button = UIButton.jx.small(type: .red, height: 200)
