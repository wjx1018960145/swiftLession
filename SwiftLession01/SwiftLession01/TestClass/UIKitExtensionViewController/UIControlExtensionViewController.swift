...
class UIControlExtensionViewController: TestBaseViewController {
...
extension UIControlExtensionViewController {
    ...
    @objc func test21() {
        ...
        let btn = UIButton(frame: CGRect(x: 50, y: jx_kScreenH - 250, width: 200, height: 100))
        btn.backgroundColor = .randomColor
        btn.addTarget(self, action: #selector(click), for: .touchUpInside)
        btn.tag = 20
        btn.setTitle("\(hitTime)秒内不可重复点击", for: .normal)
        btn.jx.preventDoubleHit(hitTime)
        btn.jx.setHandleClick { (btn) in
