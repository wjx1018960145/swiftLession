...
class UICollectionViewExtensionViewController: TestBaseViewController {
    ...
    lazy var collectionView: UICollectionView = {
        ...
        collection.jx.register(cellClass: UICollectionViewCell.self)
        return collection
    }()
...
extension UICollectionViewExtensionViewController {
    ...
    @objc func test25() {
        ...
        collectionView.jx.register(nib: nib)
    }
    
    // MARK: 2.4、注册自定义cell
    @objc func test24() {
        collectionView.jx.register(cellClass: UICollectionViewCell.self)
    }
    
    // MARK: 2.3、滚动到什么位置（CGPoint）
    @objc func test23() {
        collectionView.jx.scrollToOffset(offsetX: 20, offsetY: 100, animated: true)
    }
    
    // MARK: 2.2、是否滚动到底部
    @objc func test22() {
        collectionView.jx.scrollToBottom(animated: true)
    }
    
    // MARK: 2.1、是否滚动到顶部
    @objc func test21() {
        collectionView.jx.scrollToTop(animated: true)
    }
}
...
extension UICollectionViewExtensionViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.jx.dequeueReusableCell(cellType: UICollectionViewCell.self, cellForRowAt: indexPath)
