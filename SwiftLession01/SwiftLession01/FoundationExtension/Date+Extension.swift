...
extension Date: JXPOPCompatible {}
/// 时间戳的类型
public enum JXTimestampType: Int {
    /// 秒
    case second
    /// 毫秒
    case millisecond
...
// MARK: - 一、Date 基本的扩展
public extension JXPOP where Base == Date {
...
public extension JXPOP where Base == Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampInt = timestamp.jx.toInt() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
    ...
    /// - Returns: 返回转化后的时间戳
    static func formatterTimeStringToTimestamp(timesString: String, formatter: String, timestampType: JXTimestampType = .second) -> String {
    ...
    ///   - type: 格式类型
    /// - Returns: 返回时间条
    static func getFormatPlayTime(seconds: Int, type: JXTimeBarType = .normal) -> String {
        if seconds <= 0{
            return "00:00"
        }
        // 秒
    ...
    /// - Returns: 时间戳
    func dateToTimeStamp(timestampType: JXTimestampType = .second) -> String {
        // 10位数时间戳 和 13位数时间戳
...
public extension JXPOP where Base == Date {
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = Base.jx.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = Base.jx.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
...
public extension JXPOP where Base == Date {
    ...
    static func currentMonthDays() -> Int {
        return daysCount(year: Date.jx.currentDate.jx.year, month: Date.jx.currentDate.jx.month)
    }
}
