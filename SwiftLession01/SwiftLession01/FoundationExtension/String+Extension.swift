...
// MARK: - 一：字符串基本的扩展
public extension JXPOP where Base: ExpressibleByStringLiteral {
...
extension JXPOP where Base: ExpressibleByStringLiteral {
    ...
    public var isValidIDCardNumStrict: Bool {
        ...
        case 15:
            ...
            year = Int(str.jx.sub(start: 6, length: 2))!
        ...
        case 18:
            // 18位身份证
            year = Int(str.jx.sub(start: 6, length: 4))!
            ...
            if numberOfMatch > 0 {
                ...
                for i in 0 ..< 17 {
                    if let d = Int(str.jx.slice(i ..< (i + 1))) {
                        s += d * jiaoYan[i % 10]
                    } else {
                        return false
                    }
                ...
                let Y = s % 11
                let JYM = "10X98765432"
                let M = JYM.jx.sub(start: Y, length: 1)
                if M == str.jx.sub(start: 17, length: 1) {
                    return true
                } else {
                    return false
                }
    ...
    public func stringWithHighLightSubstring(keyword: String, normalColor: UIColor, keywordCololor: UIColor, isSplit: Bool = false, options: NSRegularExpression.Options = []) -> NSMutableAttributedString {
        ...
        if isSplit {
            for i in 0..<keyword.count {
                let singleString = keyword.jx.sub(start: i, length: 1)
                let ranges = JXRegexHelper.matchRange(totalString, pattern: singleString)
                for range in ranges {
                    attributedString.addAttributes([.foregroundColor: keywordCololor], range: range)
                }
            }
        } else {
            let ranges = JXRegexHelper.matchRange(totalString, pattern: keyword)
            for range in ranges {
                attributedString.addAttributes([.foregroundColor: keywordCololor], range: range)
            }
        }
...
extension JXPOP where Base: ExpressibleByStringLiteral {
    ...
    public func nsRange(of subString: String) -> NSRange? {
        guard (base as! String).jx.contains(find: subString) else {
            return nil
        }
    ...
    public func insertString(content: String, locat: Int) -> String {
        ...
        let str1 = (base as! String).jx.sub(to: locat)
        let str2 =  (base as! String).jx.sub(from: locat + 1)
...
public extension JXPOP where Base: ExpressibleByStringLiteral {
    ...
    func binaryToOctal() -> String {
        ...
        // 八进制
        return decimal.jx.decimalToOctal()
    }
    ...
    func binaryToHexadecimal() -> String {
        ...
        let decimal = binary.binaryTodecimal()
        // 十六进制
        return decimal.jx.decimalToHexadecimal()
    }
    ...
    func octalTobinary() -> String {
        ...
        // 二进制
        return decimal.jx.decimalToBinary()
    }
    ...
    func octalToHexadecimal() -> String {
        ...
        // 十进制
        let decimal = octal.jx.octalTodecimal()
        // 十六进制
        return decimal.jx.decimalToHexadecimal()
    }
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.base as! String).jx.toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.base as! String).jx.toInt() else {
            return ""
        }
        /*
...
public extension JXPOP where Base == String {
