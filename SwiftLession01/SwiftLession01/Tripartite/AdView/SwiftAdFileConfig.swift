...
extension SwiftAdFileConfig {
    ...
    private class func downLoadImageDataWith(_ adModel: AdFileModel) {
        ...
        ImageDownloader.default.downloadImage(
            with: url,
            options: nil,
            progressBlock: { reciveData, allData in
                print("Download Progress Image = \(Float64(reciveData)/Float64(allData))")})
        {
            ...
            if result != nil {
                print("开屏图片广告下载成功。\(String(describing: result))")
                if url.absoluteString.hasSuffix(".ceb") { /// 加密走这里
                    ...
                    let _ = SwiftAdFileConfig.saveDataToLocal(url.absoluteString, data: result as? Data)
                    /// 存入当前需要展示的广告model
                    let dataType = UIImage.checkImageDataType(data: result as? Data)
...
//
        let destination: DownloadRequest.Destination = { _, _ in
            ...
            let videoPath = filePath +  "/\(adModel.adUrl.md5() + ".mp4")"
            let fileUrl = URL(fileURLWithPath: videoPath)
            return (fileUrl, [.removePreviousFile, .createIntermediateDirectories])
        }
        AF.download(adModel.adUrl, to: destination)
