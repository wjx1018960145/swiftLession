        ...
        if url.hasSuffix(".ts") {
            ...
            /// 在这里可以对ts链接进行各种处理，（后台ts文件和m3u8文件在同一目录下）
                ...
                if let url = URL(string: newUrl) {
                    ...
                    let header = self.httpHeaderFields?["AVURLAssetHTTPHeaderFieldsKey"] as! HTTPHeaders
                    
                    let request = AF.request(newUrl, method: .get, parameters: nil, encoding: URLEncoding.default, headers: header ).responseData { (response) in
