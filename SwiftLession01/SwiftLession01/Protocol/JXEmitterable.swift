...
public protocol JXEmitterable {
}

public class JXEmitterStyle: NSObject {
    /************* 粒子发射器 ********************/
    /// 开启三维效果
...
public extension JXEmitterable where Self : UIViewController {
    ...
    ///   - style: 发射器和粒子的样式
    @discardableResult
    func startEmitter(emitterImageNames: [String], style: JXEmitterStyle = JXEmitterStyle()) -> CAEmitterLayer {
        // 1、发射器的设置
        // 1.1、创建发射器
    ...
    ///   - style: 发射器和粒子的样式
    /// - Returns: 粒子数组
    private func createEmitterCell(emitterImageNames: [String], style: JXEmitterStyle) -> [CAEmitterCell] {
        // 粒子单元数组
