...
/**
 ...
 m44:初始为1
 */
extension JXPOP where Base: UIView {
...
public extension JXPOP where Base: UIView {
    ...
    func drawCircle(fillColor: UIColor, strokeColor: UIColor, strokeWidth: CGFloat) {
        let ciecleRadius = self.base.jx.width > self.base.jx.height ? self.base.jx.height : self.base.jx.width
    ...
    func drawDashLine(strokeColor: UIColor,
                       lineLength: Int = 4,
                      lineSpacing: Int = 4,
                        direction: JXDashLineDirection = .horizontal) {
        // 线粗
        let lineWidth = direction == .horizontal ? self.base.jx.height : self.base.jx.width
        ...
        if direction == .horizontal {
            ...
            // 终点
            // 横向 y = lineWidth / 2
            path.addLine(to: CGPoint(x: self.base.jx.width, y: lineWidth / 2))
        } else {
            path.move(to: CGPoint(x: lineWidth / 2, y: 0))
            // 终点
            // 纵向 Y = view 的height
            path.addLine(to: CGPoint(x: lineWidth / 2, y: self.base.jx.height))
        }
...
// MARK: - 七、其他的方法
// 抖动方向枚举
public enum JXShakeDirection: Int {
...
public extension JXPOP where Base: UIView {
    ...
    func addWater(markText: String, textColor: UIColor = UIColor.black, font: UIFont = UIFont.systemFont(ofSize: 12)) {
        let waterMark: NSString = markText.jx.toNSString
        let textSize: CGSize = waterMark.size(withAttributes: [NSAttributedString.Key.font : font])
        // 多少行
        let line: NSInteger = NSInteger(self.base.jx.height * 3.5 / 80)
        // 多少列：自己的宽度/(每个水印的宽度+间隔)
        let row: NSInteger = NSInteger(self.base.jx.width / markText.jx.rectWidth(font: font, size: CGSize(width: self.base.jx.width, height: CGFloat(MAXFLOAT))))
        for i in 0..<line {
            for j in 0..<row {
    ...
    func shake(direction: JXShakeDirection = .horizontal, times: Int = 3, interval: TimeInterval = 0.1, delta: CGFloat = 2, completion: (() -> Void)? = nil) {
        // 播放动画
    ...
    func isContainsWKWebView() -> Bool {
        ...
        for subView in base.subviews {
            if (subView.jx.isContainsWKWebView()) {
                return true
            }
        }
        return false
...
public extension JXPOP where Base: UIView {
    ...
    func getViewLayer(borderWidth: CGFloat = 0.5, borderColor: UIColor = .randomColor, backgroundColor: UIColor = .randomColor) {
        ...
        for subview in subviews {
            ...
            subview.jx.getViewLayer(borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }
        #endif
    }
    ...
    func findSubview(type: UIResponder.Type, resursion: Bool)-> UIView? {
        ...
        for e in self.base.subviews.enumerated() {
            let tmpView = e.element.jx.findSubview(type: type, resursion: resursion)
            if tmpView != nil {
                return tmpView
            }
        }
...
public extension JXPOP where Base: UIView {
    ...
    func addActionClosure(_ actionClosure: @escaping ViewClosure) {
        if let sender = self.base as? UIButton {
            sender.jx.setHandleClick(controlEvents: .touchUpInside) { (control) in
            ...
            sender.jx.addActionHandler({ (control) in
                actionClosure(nil, control, control.tag)
            }, for: .valueChanged)
        } else {
            _ = self.base.jx.addGestureTap { (reco) in
...
public extension JXPOP where Base : UIView {
    ...
    func gradientColor(_ direction: JXViewGradientDirection = .horizontal, _ gradientColors: [Any], _ gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().jx.gradientLayer(direction, gradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.jx.width, height: self.base.jx.height)
    ...
    ///   - gradientLocations: 决定每个渐变颜色的终止位置，这些值必须是递增的，数组的长度和 colors 的长度最好一致
    func gradientColorAnimation(direction: JXViewGradientDirection = .horizontal, startGradientColors: [Any], endGradientColors: [Any], duration: CFTimeInterval = 1.0, gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().jx.gradientLayer(direction, startGradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.jx.width, height: self.base.jx.height)
        self.base.layer.insertSublayer(gradientLayer, at: 0)
