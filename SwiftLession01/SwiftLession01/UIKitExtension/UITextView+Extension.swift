...
extension UITextView {
    ...
    fileprivate func initPlaceholder(_ placeholder: String) {
        ...
        let label = UILabel()
        let width = self.jx.width - placeholderOrigin!.x * 2 - 1
...
// MARK: - 二、文本链接的扩展
public extension JXPOP where Base: UITextView {
    ...
    func resolveHashTags() {
        ...
        for sentence in sentences {
            // 如果是url链接则跳过
            if !(sentence as String).jx.verifyUrl() {
                // 再按特殊符号拆分
    ...
    func inputRestrictions(shouldChangeTextIn range: NSRange, replacementText text: String, maxCharacters: Int, regex: String?) -> Bool {
        ...
        if let _ = self.base.markedTextRange {
            ...
            if range.length == 0 {
                ...
                // 正则的判断
                if let weakRegex = regex, !JXRegexHelper.match(text, pattern: weakRegex) {
                    return false
                }
                // 联想选中键盘
                let allContent = oldContent.jx.sub(to: range.location) + text
                if allContent.count > maxCharacters  {
                    let newContent = allContent.jx.sub(to: maxCharacters)
            ...
            guard !text.jx.isNineKeyBoard() else {
                return true
            }
            // 正则的判断
            if let weakRegex = regex, !JXRegexHelper.match(text, pattern: weakRegex) {
