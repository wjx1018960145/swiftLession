...
import Foundation
import UIKit
extension CALayer: JXPOPCompatible {}
// MARK: - 一、自定义链式编程
...
public extension JXPOP where Base: CALayer {
...
public extension JXPOP where Base: CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func addKeyframeAnimationRotation(values: [Any] = [CGFloat(-5).jx.degreesToRadians(), CGFloat(5).jx.degreesToRadians(), CGFloat(-5).jx.degreesToRadians()],
                                      keyTimes: [NSNumber]?,
