...
// MARK: - 一、基本的扩展
public extension JXPOP where Base: UIButton {
...
public extension UIButton {
    ...
    func image(_ color: UIColor, _ size: CGSize = CGSize(width: 20.0, height: 20.0), _ state: UIControl.State = .normal) -> Self {
        let image = UIImage.jx.image(color: color, size: size)
        setImage(image, for: state)
        return self
    }
    ...
    func bgImage(_ color: UIColor, _ state: UIControl.State = .normal) -> Self {
        let image = UIImage.jx.image(color: color)
        setBackgroundImage(image, for: state)
        return self
    }
    ...
    func confirmButton() -> Self {
        let normalImage = UIImage.jx.image(color: UIColor.hexStringColor(hexString: "#E54749"), size: CGSize(width: 10, height: 10), corners: .allCorners, radius: 4)?.resizableImage(withCapInsets: UIEdgeInsets(top: 5, left: 5, bottom: 5, right: 5))
        let disableImg = UIImage.jx.image(color: UIColor.hexStringColor(hexString: "#E6E6E6"), size: CGSize(width: 10, height: 10), corners: .allCorners, radius: 4)?.resizableImage(withCapInsets: UIEdgeInsets(top: 5, left: 5, bottom: 5, right: 5))
...
extension UIButton: JXSwiftPropertyCompatible {
    internal typealias T = UIButton
    internal var swiftCallBack: SwiftCallBack? {
        get { return jx_getAssociatedObject(self, &buttonCallBackKey) }
        set { jx_setRetainedAssociatedObject(self, &buttonCallBackKey, newValue) }
    }
...
// MARK: - 六、Button扩大点击事件
private var JXUIButtonExpandSizeKey = "JKUIButtonExpandSizeKey"
public extension UIButton {
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let buttonRect = self.jx.expandRect()
...
public extension JXPOP where Base: UIButton {
    ...
    func expandSize(size: CGFloat) {
        objc_setAssociatedObject(self.base, &JXUIButtonExpandSizeKey, size, objc_AssociationPolicy.OBJC_ASSOCIATION_COPY)
    }

    fileprivate func expandRect() -> CGRect {
        let expandSize = objc_getAssociatedObject(self.base, &JXUIButtonExpandSizeKey)
        if (expandSize != nil) {
